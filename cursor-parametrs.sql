...
DECLARE
    --DECLARE THE CURSOR
    CURSOR emp_cursor (
        select_department departments.department_id%TYPE
    ) IS
    SELECT
        *
    FROM
...
BEGIN
    ...
    IF NOT ( emp_cursor%isopen ) THEN
        --RETRIEVE THE CURSOR PARAMETER VALUE 
        SELECT
            d.department_id
        INTO select_department
        FROM
            departments   d
        ...
        --OPEN THE CURSOR AND PASS THE PARAMETER TO CURSOR        

        OPEN emp_cursor(select_department);
    END IF;
